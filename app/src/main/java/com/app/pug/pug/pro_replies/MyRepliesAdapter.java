...
public class MyRepliesAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    public static HashMap<Integer, Integer> detect = new HashMap<>();
    int times = 0;
    private AppCompatActivity activity;

    public MyRepliesAdapter(AppCompatActivity activity) throws JSONException {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType==1) {
            ...
            final Posts holder = new Posts(view, new Posts.MyViewclickHolder() {
                    ...
                    if (ProfileGetPosts.posts.size() != 0) {
                        ...
                        try {
                            if (!ProfileGetPosts.posts.get(pos).isNull("self") && ProfileGetPosts.posts.get(pos).getBoolean("self")) {
                                ft.replace(R.id.my_replies, IndividualPosts.newInstance(pos, "my_replies", "my_replies", true, ProfileGetPosts.posts.get(pos).getBoolean("can_reply")));

                            } else {
                                ft.replace(R.id.my_replies, IndividualPosts.newInstance(pos, "my_replies", "my_replies", false, ProfileGetPosts.posts.get(pos).getBoolean("can_reply")));
                ...
                public void onHeartPress(View imgB, int pos) {
                    if (MyRepliesFrag.integerHashMap.get(pos) == 0) {
                        MyRepliesFrag.integerHashMap.set(pos, 1);
                        detect.put(pos, 1);
                ...
                public void onImagePressed(View v, int pos) {
                    FragmentTransaction ft=activity.getSupportFragmentManager().beginTransaction();
                    ft.setCustomAnimations(R.anim.push_left_in, R.anim.push_left_out, R.anim.push_right_in, R.anim.push_right_out);
                    ft.replace(R.id.my_replies, SingleImageView.newInstance(pos, "my_replies"));
                    ft.addToBackStack(null);
                    ft.commit();
                }
            });
        ...
        {
            View view=LayoutInflater.from(parent.getContext()).inflate(R.layout.myreplies_no_post,parent,false);
            NoPosts noPosts=new NoPosts(view);
            return noPosts;
        }
        else
        {
            throw new RuntimeException("Error finding a ViewHolder");
        }
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder viewHolder, int position)
    {
        if (viewHolder instanceof Posts) {
            ...
            if (MyRepliesFrag.integerHashMap.get(position) == 1)
            {
            ...
            else if (MyRepliesFrag.integerHashMap.get(position) == -1) {
                if (detect.containsKey(position)) {
                    if (detect.get(position) == -1) {
                        try {
                            int oldLike = ProfileGetPosts.posts.get(position).getInt("likes");
            ...
            else if (MyRepliesFrag.integerHashMap.get(position) == 0) {
                try {
                    posts.likes.setText(ProfileGetPosts.posts.get(position).getString("likes"));
                } catch (JSONException e) {
                    e.printStackTrace();
